		...
		if (action != null) {
			if (action.equals("verParametros")) {
				...
				if (codCurso != null && seccion != null) {
					...
					curso = cm.obtenerParametros(to.getCursoId(), to
							.getSeccion());
				}
				...
				cm.eliminarTipoPrueba(curso.getCursoId(), curso.getSeccion(),
						codTipPba, user.getCodUsr());
				...
				if (codDoc != null) {// actualizar el curso
					CursoManager cm = new CursoManager(getDataSource(request,
							"sisNotas"));
					CursoTO curso = (CursoTO) session
							.getAttribute("paramCurso");
					cm.updateDocente(codDoc, curso.getCursoId(), curso
							.getSeccion());
				}
				...
				if (codDoc != null) {
					if (!codDoc.equals("")) {
						CursoManager cm = new CursoManager(getDataSource(
								request, "sisNotas"));
						CursoTO curso = (CursoTO) session
								.getAttribute("paramCurso");
						cm.updateDocente(codDoc, curso.getCursoId(), curso
				...
				ParametrosManager pm = new ParametrosManager(getDataSource(
						request, "sisNotas"));
				...
				TipoPruebaTO to2 = cm.find(curso.getCursoId(), curso
						.getSeccion(), to);
				if (to2 == null) {
					log.info("inserto");
				...
				PruebaManager pm = new PruebaManager(getDataSource(
						request, "sisNotas"));
