...
public class CrearAdministradorAction extends Action {
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if (((AdministradorForm) form).getPassword().equals(
				((AdministradorForm) form).getPassword2())) {
			HttpSession session = request.getSession();
			// objeto user de la sesion
			UsuarioTO user = (UsuarioTO) session.getAttribute("usuario");
			// sesion
			UsuarioTO admin = new UsuarioTO();
			UsuarioManager userManager = new UsuarioManager(getDataSource(
					request, "sisNotas"));
			...
			if (userManager.userNameExist(admin.getUserName())) {
				...
				errors.add("username", new ActionError(
						"error.regAdmin.username.userExist"));
